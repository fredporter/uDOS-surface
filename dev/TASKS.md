...
| **T-ALPHA-UOS** | External app launcher (`uos`) | [`modules/uos/`](../modules/uos/) — `udo app list`; `udo app launch <app> [args…]` (dry-run) / `udo app launch <app> --execute [args…]` runs `docker`/`podman` per OBX; **next:** `{file}` vs workspace mount semantics, optional runtime override |
