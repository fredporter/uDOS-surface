...
- **A1** and **A2** are **alpha** milestones for **uDosConnect**: A1 = local-first wireframe core and CLI; A2 = always-on / network / universe lane (see `docs/specs/workflow-network-a1-a2.md`, `docs/specs/version-ladder-a1-a2.md`).
...
## Architecture rules (uDosConnect only)

- **Governance:** **`dev/`** only. **Documentation corpus:** **`docs/`** only. No root **`uDosDev/`** / **`uDosDocs/`** folders.
- **Sync:** `git pull` in **uDosConnect**; no submodules. **Checks:** `scripts/v4-dev/check-tasks-md.sh` validates task files in this repo only; **`scripts/shakedown.sh`** for umbrella validation.
