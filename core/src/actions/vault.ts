...
export async function cmdInit(): Promise<void> {
  ...
  const dirs = ["content", "system", "system/usxd", "spool", "feeds", ".compost", "ucode"];
  for (const d of dirs) {
...
export async function cmdList(): Promise<void> {
  ...
  if (!(await fs.pathExists(root))) {
    console.error(chalk.red("No vault — run: do init"));
    process.exitCode = 1;
    return;
  }
...
export async function cmdEdit(file: string): Promise<void> {
  console.log(chalk.yellow("VA1: use `do open <file>` with $EDITOR, or run your editor directly."));
  await cmdOpen(file);
}

export async function cmdDelete(file: string): Promise<void> {
  const root = getVaultRoot();
  const id = await moveToCompost(root, file);
...
export async function cmdRestore(id: string): Promise<void> {
  ...
  const dest = await restoreFromCompost(root, id);
